...
void Solver::Solve(QString& s1,QString& s2,QString& s3,QString& s4,QString&s5,QTcpSocket*socket)
{
    vector<QString>s;
    ...
    {
        s.push_back("11");
        vector<QString>evenimente=db.GetEvents(s2);
        s.push_back(QString::number(evenimente.size()));
        for(QString c:evenimente)
            s.push_back(c);
    }
    else if(s1=="12")
    {
    ...
    {
        s.push_back("13");
        QString notificare=db.AddPost(s2,s3,s4);
        if(notificare!=""){
            s.push_back("1");
            s.push_back(notificare.toUtf8());
        }
        else
            s.push_back("0");
    }
