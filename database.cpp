...
int Database::Login(const QString& username,const QString& parola)
{
    ConnectToDB();
...
int Database::Register(const QString& username,const QString& parola,const QString& email)
{
    ConnectToDB();
...
int Database::ChangePassword(const QString& username,const QString& email)
{
    ConnectToDB();
...
vector<QString> Database::GetNotifications(const QString& username)
{
    ConnectToDB();
...
int Database::CreateChannel(const QString& ChannelName,const QString& Descriere,const QString& Owner)
{
    ConnectToDB();
...
vector<QString> Database::GetChannels(const QString& username)
{
     ConnectToDB();
...
vector<QString> Database::GetPosts(const QString& canal)
{
    ConnectToDB();
    vector<QString>mesaje;
...
int Database::AddMember(const QString& Channel,const QString& Owner,const QString& User)
{
    ConnectToDB();
    ...
    while(q.next())
    {
        ...
        q2.exec();
        q2.finish();
        QString notificare="["+QDateTime::fromSecsSinceEpoch(time(0)).toString()+"]"+Owner+" "+"v-a adaugat in canalul "+Channel;
        QSqlQuery ask;
        ...
        for(pair<QString,QTcpSocket*>i:usersconnected)
        {
            if(i.first==User&&i.first!=Owner){
                i.second->write(("101|"+notificare).toUtf8());
                i.second->waitForBytesWritten(5000);
                i.second->write(("104|"+Channel).toUtf8());
                break;
            }
        }
        q.finish();
...
vector<QString> Database::GetUsersFromChannel(const QString& canal)
{
    ConnectToDB();
...
vector<QString> Database::GetFilesFromChannel(const QString& canal)
{
    ConnectToDB();
...
void Database::AddMessageInConversation(const QString&source,const QString&destination,const QString&message)
{
    ...
    while(q.next())
    {
        ...
        if(dest_socket!=nullptr){
            dest_socket->write(("103|"+source+"|"+destination+"|"+mes).toUtf8());
            dest_socket->waitForBytesWritten(5000);
...
QString Database::AddPost(const QString&Channel,const QString&post,const QString&owner)
{
    ConnectToDB();
    QString s("INSERT INTO Mesaje_Canale(Expeditor,Text,Titlu,Timp_incarcare,ID_Canal) VALUES ('"+owner+"','"+post+"','-',"+QString::number(time(0))+",(SELECT ID_Canal From Canale WHERE Nume_Canal='"+Channel+"'))");
    QSqlQuery q;
    q.prepare(s);
    q.exec();
    QSqlQuery ask;
    ...
    ask.finish();
    QString notificare="["+QDateTime::fromSecsSinceEpoch(time(0)).toString()+"]"+owner+": "+post;
    for(QString&user:useri)
        for(pair<QString,QTcpSocket*>p:TcpServer::getInstance().getUsers())
            if(user==p.first&&user!=owner)
            {
                p.second->write(("102|"+notificare+"|"+Channel).toUtf8());
            }
    Disconnect();
    return notificare;
}
vector<QString> Database::GetEvents(const QString&user)
{
    ConnectToDB();
    QString s("SELECT * FROM Evenimente WHERE Nume='"+user+"' ORDER BY Data");
    QSqlQuery q;
    q.prepare(s);
    q.exec();
    vector<QString>ev;
    while(q.next())
       {
            QString eveniment=q.value(0).toString()+" a programat meetul "+q.value(1).toString()+" in canalul '"+q.value(2).toString()+"' pe data de ["+q.value(3).toDateTime().toString()+"]";
            ev.push_back(eveniment);
       }
    return ev;
}
int Database::AddFile(const QString&channel,const QString&Owner,const QString&Nume,const QString&continut)
{
    vector<QString>files=GetFilesFromChannel(channel);
    qDebug()<<files.size();
    for(QString f:files){
        qDebug()<<Nume<<f;
        if(f==Nume)
    ...
    ConnectToDB();
    QString s("INSERT INTO Fisiere(Nume_Fisier,Nume_Autor,Timp_Incarcare,ID_Canal,Continut) VALUES ('"+Nume+"','"+Owner+"',"+QString::number(time(0))+",(SELECT ID_Canal From Canale Where Nume_Canal='"+channel+"'),CONVERT(VARBINARY(MAX),N'"+continut+"'))");
    qDebug()<<s;
    QSqlQuery q;
    q.prepare(s);
    q.exec();
...
vector<QString> Database::downloadFile(const QString&channel,const QString&file)
{
    ConnectToDB();
    QString s("SELECT Nume_Fisier,CONVERT(NVARCHAR(MAX),Continut) FROM Fisiere WHERE ID_Canal=(SELECT ID_Canal FROM Canale WHERE Nume_Canal='"+channel+"')");
    QSqlQuery q;
    q.prepare(s);
    q.exec();
    vector<QString>ToEncode;
    while(q.next())
    {
        ToEncode.push_back("15");
        ToEncode.push_back(file);
        ToEncode.push_back(q.value(1).toString());
